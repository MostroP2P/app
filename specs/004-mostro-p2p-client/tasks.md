...
- [x] T063 [P] Implement pay invoice widget (manual QR mode) in `lib/shared/widgets/pay_lightning_invoice_widget.dart`: QR code display using `qr_flutter`, copy button, share button (system share sheet). `onSubmit` (user confirms manual payment), `onCancel` callbacks.
...
- [x] T066 Implement release order action in `rust/src/api/orders.rs`: add `release_order(order_id)` — sends `Release` `MostroMessage`. Updates trade status to `SettledHoldInvoice` → `Success`. Streams `on_trade_updated(order_id)`.
- [x] T067 Wire seller active view in trade detail: active status + seller role → show CLOSE + CANCEL + DISPUTE + CONTACT (no RELEASE, no FIAT SENT). Seller card 5 instruction: "Contact the buyer [handle] with payment instructions." Status: "Active order".
- [x] T068 Wire seller release flow: RELEASE tap → confirmation dialog → Yes → `release_order()` → reactive button → on Success → navigate to rate screen `/rate_user/:orderId`.
