...
**Checkpoint**: P2P text chat works end to end — `send_message` (NIP-59 publish) + `subscribe_incoming_chat` (receive → store → `on_new_message`), peer ECDH key derivation, and the Blossom client (Kind-24242 auth) are all done. **Still pending**: the file-attachment UI (send + display/download) is not wired to the bridge — `send_file`/`download_attachment` have no Dart caller (T079/T080/T081 + the chat `_onAttach` hook).
...
- [~] T090 Implement dispute chat screen in `lib/features/disputes/screens/dispute_chat_screen.dart`: header section with "Dispute with Buyer/Seller: [handle]" + status badge "In progress" (green) + order UUID + dispute UUID + instructional text about shared key. Chat area (same bubble system: own=purple `#7856AF`, admin=dark gray). Message input (only shown when `status == in-progress`; hidden for resolved/closed disputes). Route: `/dispute_details/:disputeId`. On `initState` → mark as read. **Partial**: screen scaffold built, but the message list is stubbed (dispute_chat_screen.dart) — it does not subscribe to `on_dispute_updated` and does not consume the `adminSharedKey` (T086), so admin chat (send + receive) is not functional.
- [~] T091 [P] Implement dispute messages list in `lib/features/disputes/widgets/dispute_messages_list.dart`: SliverList combining: `DisputeInfoCard` (first always), optional "Admin assigned" blue card (in-progress + no messages), `DisputeMessageBubble` entries (sorted by timestamp, deduped by event ID), optional "Chat closed" lock banner (resolved/seller-refunded/closed). Auto-scroll to bottom on new message. **Partial**: widget built but fed stub data — no real dispute messages reach it because the screen/provider are not wired to the bridge (see T090).
- [~] T092 [P] Implement dispute message input in `lib/features/disputes/widgets/dispute_message_input.dart`: same pattern as P2P `MessageInput` (attach + text + send). Attachment uses `adminSharedKey` for encryption. Show spinner while uploading. Only rendered when dispute `status == in-progress`. **Partial**: input widget built, but neither send nor attach is wired — `_onAttachFile` is a stub and sending needs the `adminSharedKey` handshake (T086) connected to the screen.
- [~] T093 Implement dispute resolved screen in `lib/features/disputes/screens/dispute_chat_screen.dart` (terminal states): when dispute `status == resolved` (admin settled buyer) → green checkmark + "successfully completed" text + lock icon + "This dispute has been resolved. The chat is closed." When `status == seller-refunded` (admin canceled) → "Resolved" badge (blue) + green resolution box "The administrator canceled the order and refunded you. The buyer did not receive the sats." + lock message. No action buttons except back arrow. **Partial**: terminal-state UI built but currently unreachable — dispute status is never populated from the bridge (see T088/T090), so the resolved / seller-refunded branches never trigger at runtime.
