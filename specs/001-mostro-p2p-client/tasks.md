...
- [ ] T017 Implement order state machine in rust/src/protocol/state_machine.rs: all 15 states per mostro-core (Pending, WaitingBuyerInvoice, WaitingPayment, Active, FiatSent, SettledHoldInvoice, Success, Canceled, Expired, CooperativelyCanceled, CanceledByAdmin, SettledByAdmin, CompletedByAdmin, Dispute, InProgress), valid transitions, and action-to-state mapping per protocol reference. Note: PaymentFailed is an Action notification, not a Status.
