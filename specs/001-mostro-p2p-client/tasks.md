...
- [ ] T061 [P] [US5] Implement nym avatar widget in lib/widgets/nym_avatar.dart: call get_nym_identity(pubkey), render pseudonym text label, select icon by icon_index (u8 0–36 maps to icon set), fill avatar circle background with HSV color from color_hue (u16 0–359), ALWAYS render icon in white (Colors.white) regardless of color_hue — v1 bug fix where icon was invisible against matching background (FR-011c, FR-052/FR-053 and contracts/types.md NymIdentity)
...
- [ ] T085 [US7] Implement settings API in rust/src/api/settings.rs: get_settings(), set_theme(ThemeMode), set_language(locale), set_default_fiat_code(code?), set_default_lightning_address(address?), set_logging_enabled(bool), on_settings_changed() stream per contracts/settings.md — privacy_mode is read-only in Settings; use set_privacy_mode() from the Reputation API (rust/src/api/reputation.rs) to change it
...
- [ ] T090 [P] [US7] Implement preferences screen in lib/screens/settings/preferences_screen.dart: privacy mode toggle (Reputation Mode / Full Privacy), theme selector (System/Dark/Light chips), language picker (exactly 5 locales: en, es, it, fr, de per FR-020d), default fiat currency dropdown, Lightning Address text field per US7 scenarios 7–10 and FR-020d/FR-044/FR-054/FR-055
...
- [ ] T095 [P] [US7] Implement About screen in lib/screens/settings/about_screen.dart: app version, commit hash, GitHub link (tappable), MIT license (tappable modal), documentation links (user guide EN, user guide ES, technical reference — each opens external URL), Mostro node card (pubkey, version, fee%, min/max order sats, supported currencies, LN node ID + alias) with info-icon tooltips per US7 scenarios 14–15 and FR-020b/FR-057/FR-058
