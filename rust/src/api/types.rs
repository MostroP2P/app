...
pub struct IdentityInfo {
    ...
    /// words (issue #141 — migrated out of Dart SharedPreferences into the Rust
    /// identity record per Principle I). `#[serde(default)]` so identities
    /// persisted before this field deserialize as `false` — an unconfirmed
    /// backup, which correctly keeps the reminder armed.
    #[serde(default)]
