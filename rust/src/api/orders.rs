        ...
        Some(mut info) => {
            ...
            if info.status != crate::api::types::OrderStatus::Pending {
                ...
                if info.is_mine {
                    ...
                    // Push the synced status so a 38383-only transition (no
                    // accompanying kind-14) reaches a live tradeStatusProvider
                    // immediately, instead of waiting for the 30s reconciliation
                    // tick (#272 scope item 4). Gated on `applies` — the same
                    // forward-only guard the DB write above uses — so a push
                    // never walks status backwards.
                    if applies {
                        emit_trade_update(&info.id, info.status.clone());
                    }
                }
                if !applies {
                    if let Some(local) = local {
